/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Tue Dec  5 23:35:22 2017
 ************************************************************************/
...
void static udpserv_echo(int s, struct sockaddr * client) {
 ...
 //       n = recvfrom(s, buff, BUFF_LEN, 0, client, &len);
   //     printf("The server recved <%d>: %s size: %d\n", ++count, buff, n);
        
        n = recvfrom(s, buff, 100, 0, client, &len);
        printf("No.1 The server recved size: %d\n", n);
        n = recvfrom(s, buff, 1024, 0, client, &len);
        printf("No.2 The server recved size: %d\n", n);
        n = recvfrom(s, buff, 1024, 0, client, &len);
        printf("No.3 The server recved size: %d\n", n);
    }
}
...
int main(int argc, char * argv[]) {
    ...
    udpserv_echo(s, (struct sockaddr*)&addr_clie);//数据接收处理程序
    return 0;
}
